...
needed_packages = ["pandas", "numpy", "seaborn", "matplotlib"]
...
parser.add_argument(
    "-o", "--outplotname", required=True, help="File where the TMB plot should be saved"
)
...
ax = sns.stripplot(
    x="Histology_type",
    y="TMB",
    data=tmb_scores_df.sort_values("TMB"),
    jitter=1,
    linewidth=0.5,
)
...
for tick, text in zip(ax.get_xticks(), ax.get_xticklabels()):
    ...
    median_val = np.median(
        tmb_scores_df["TMB"][tmb_scores_df["Histology_type"] == sample_name]
    )
