...
run_deconstructSigs <- function( maf = maf,
                              genomeversion = "hg38",
  ...
  if (!is.null(wgs_bed) & !is.null(wxs_bed)) {
    ...
    # So End - Start gives the size of each range.
    # Sum the gives the total size in bp.
  ...
  if ( any(normalization %in%
           c("default", "exome", "genome", "exome2genome" ))) {
  ...
  sig_num_df <- do.call(
    ...
    dplyr::left_join(metadata_df %>%
                       dplyr::select(!!(as.name(grouping_by)),
